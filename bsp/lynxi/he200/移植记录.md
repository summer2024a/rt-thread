...
- [x] Zephyr `build_he200_ep_final` 同环境对照（GMAC 已编入 `he200_ep_defconfig`）
...
验收脚本（iperf 须 **v2、端口 5001**）：

| 脚本 | 用途 |
|------|------|
| [`he200_gmac_ping_test.sh`](scripts/he200_gmac_ping_test.sh) | ping 8/8（RT/Zephyr） |
| [`he200_gmac_bw_test.sh`](scripts/he200_gmac_bw_test.sh) | UDP 带宽 + EP TX（推荐，默认跳过 TCP） |
| [`he200_gmac_tcp_test.sh`](scripts/he200_gmac_tcp_test.sh) | TCP 快测 |
| [`he200_gmac_iperf_test.sh`](scripts/he200_gmac_iperf_test.sh) | 旧版综合（先 TCP 可能触发 EP 断言，优先用 bw_test） |

**Zephyr 对照（2026-06-08，同环境）**：`build_he200_ep_final`（GMAC 编入 `he200_ep_defconfig`）ping 8/8、UDP ~52 Mbps、TCP ~0.07 Mbps；详见 `zephyr/boards/lynxi/PORTING.md` §10.6。
...
| 2026-06-08 | UDP 带宽 ~52/177 Mbps 验收；TCP ~0.07 Mbps（栈层瓶颈）；新增 `he200_gmac_bw_test.sh` / `he200_gmac_tcp_test.sh`；Zephyr 同环境对照 |

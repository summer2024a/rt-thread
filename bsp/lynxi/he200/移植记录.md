...
**仍待实板复验**（代码已合入 2026-06-06）：

- Host `enp25s0f1` `Link detected: yes`（插网线后 `anlpar` 应非 `0000`）
- `list_isr` 中 `e0_isr` counter 递增
- `ping 192.168.1.1` 稳定；Host `ping 192.168.1.2` 通
- 带宽：`iperf3` 或 lwIP 压测有合理吞吐

**环境脚本**：[`scripts/he200_test_env.sh`](scripts/he200_test_env.sh)（在 `192.168.49.81` 配置 `enp25s0f1` 为 `192.168.1.1/24`）。
...
| 2026-06-06 | RTL8211F `rgmii-id` PHY delay、`eth_rx_irq` erx 兜底、环境脚本 `scripts/he200_test_env.sh`；ping/带宽待 49.81 实板复验 |

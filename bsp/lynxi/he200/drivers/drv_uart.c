...
static int uart_getc(struct rt_serial_device *serial)
{
    ...
    // while(DW_APB_REG_LSR(uart->hw_base) & (UART_LSR_DR | UART_LSR_BI));
    // ch = DW_APB_REG_RBR(uart->hw_base);
...
void rt_hw_earlycon_ioremap_early(void)
{
    ...
    earlycon_base = rt_ioremap_early((void *)_earlycon_base, earlycon_size);
...
int rt_hw_uart_init(void)
{
    ...
    uart1_addr = UART1_BASE;
#ifdef RT_USING_SMART
    uart1_addr = (size_t)rt_ioremap((void*)UART1_BASE, 0x1000);
#endif
    earlycon_base = (void *)uart1_addr;
    uart1->hw_base = uart1_addr;
    ...
    rt_hw_serial_register(&_serial1, "uart1",
                          RT_DEVICE_FLAG_RDWR | RT_DEVICE_FLAG_INT_RX,
                          uart1);
    rt_hw_interrupt_install(uart1->irqno, rt_hw_uart_isr, &_serial1, "uart1");

...
rt_err_t cpu_set_pll_clk(int clk)
{
    ...
    n = clk / (_24MHZ_ * k / m) - 1;
    cpu_src = (CPR->cpu_clk_src >> 16) & 0x3;
    CPR->cpu_clk_src = CLK_OSC24M_SRC << 16;
    CPR->pll_cpu_ctrl &= ~(0x1 << 31);
    //PLL1 rate = ((24000000 * n * k) >> 0) / m   (p is ignored)
    CPR->pll_cpu_ctrl = (0x1 << 31) | (m << 0) | (k << 4) | (n << 8) | (p << 16);
    if (wait_pll_stable((rt_uint32_t)(&CPR->pll_cpu_ctrl)))
        return -RT_ERROR;

    CPR->cpu_clk_src = cpu_src << 16;
...
rt_err_t audio_set_pll_clk(int clk)
{
    ...
    m = (m - 1) & 0x1f;
    //clk = (24 * n * 2) / m
    CPR->pll_audio_ctrl = (0x1 << 31) | (0x0 << 24) | (n << 8) | m;

    if (wait_pll_stable((rt_uint32_t)(&CPR->pll_audio_ctrl)))
        return -RT_ERROR;
    else
        return RT_EOK;
}
...
rt_err_t video_set_pll_clk(int clk)
{
    ...
    m = (m - 1) & 0xf;
    //speed = (24*n)/m
    CPR->pll_video_ctrl = (0x1 << 31) | (0x0 << 30) | (0x1 << 24) | (n << 8) | m;

    if (wait_pll_stable((rt_uint32_t)(&CPR->pll_video_ctrl)))
        return -RT_ERROR;
    else
        return RT_EOK;
}
...
rt_err_t ve_set_pll_clk(int clk)
{
    ...
    m = (m - 1) & 0xf;
    //clk = (24 * n) / m
    CPR->pll_ve_ctrl = (0x1 << 31) | (0x1 << 24) | (n << 8) | m;
    if (wait_pll_stable((rt_uint32_t)(&CPR->pll_ve_ctrl)))
        return -RT_ERROR;
    else
        return RT_EOK;
}
...
rt_err_t periph_set_pll_clk(int clk)
{
    ...
    if ((n > 32) || (k > 4) || (clk != (_24MHZ_ * n * k)))
        return -RT_ERROR;
    temp_data = CPR->ahb_apb_hclkc_cfg;
    clk_src = (temp_data >> 12) & 0x3;
    temp_data &= ~(0x3 << 12);
    temp_data |= (CLK_PLL_SRC << 12);
    CPR->ahb_apb_hclkc_cfg = temp_data;
    CPR->pll_periph_ctrl &= ~(0x1 << 31);
    n = (n - 1) & 0x1f;
    k = (k - 1) & 0x3;
    //clk = 24 * n *k
    CPR->pll_periph_ctrl = (0x1 << 31) | (0x1 << 18) | (n << 8) | (k << 4) || (0x1);
    if (wait_pll_stable((rt_uint32_t)(&CPR->pll_periph_ctrl)))
        return -RT_ERROR;
...
rt_err_t cpu_set_clk(int clk)
{
    ...
    CPR->ahb_apb_hclkc_cfg &= ~(0x3 << 16);
    CPR->cpu_clk_src = CLK_PLL_SRC << 16;
...
rt_err_t bus_gate_clk_enalbe(enum bus_gate bus)
{
    ...
    if (gate_reg == 0x00)
        CPR->bus_clk_gating0 |= (0x1 << offset);
    else if (gate_reg == 0x01)
        CPR->bus_clk_gating1 |= (0x1 << offset);
    else if (gate_reg == 0x02)
        CPR->bus_clk_gating2 |= (0x1 << offset);
    else
        return -RT_EINVAL;
...
rt_err_t bus_software_reset_disalbe(enum bus_gate bus)
{
    ...
    if (gate_reg == 0x00)
        CPR->bus_soft_rst0 |= (0x1 << offset);
    else if (gate_reg == 0x01)
        CPR->bus_soft_rst1 |= (0x1 << offset);
    else if (gate_reg == 0x02)
        CPR->bus_soft_rst2 |= (0x1 << offset);
    else
        return -RT_EINVAL;
...
{
    CPR->dram_gating |= (0x01 << dram_gate);
    return RT_EOK;
}

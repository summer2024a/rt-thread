...
#define DWC_MSHC_PHY_PAD_EMMC_DAT \
    ((3 << TXSLEW_N_LSB) | (3 << TXSLEW_P_LSB) | (1 << WEAKPULL_EN_LSB) | (1 << RXSEL_LSB))
#define DWC_MSHC_PHY_PAD_EMMC_STB \
    ((3 << TXSLEW_N_LSB) | (3 << TXSLEW_P_LSB) | (2 << WEAKPULL_EN_LSB) | (1 << RXSEL_LSB))

#define readl(addr)           HWREG32(addr)
#define readw(addr)           HWREG16(addr)
#define readb(addr)           HWREG8(addr)
#define writel(value,addr)    (HWREG32(addr) = (value))
#define writew(value,addr)    (HWREG16(addr) = (value))
#define writeb(value,addr)    (HWREG8(addr) = (value))
...
{
    sdhci_writew(host, 0x669, DWC_MSHC_CMDPAD_CNFG);
    sdhci_writew(host, 0x669, DWC_MSHC_DATPAD_CNFG);
    sdhci_writew(host, 0x660, DWC_MSHC_CLKPAD_CNFG);
    sdhci_writew(host, 0x671, DWC_MSHC_STBPAD_CNFG);
    sdhci_writew(host, 0x669, DWC_MSHC_RSTNPAD_CNFG);
}
...
__attribute__((unused)) static void dwcmshc_phy_delay_config(struct sdhci_host* host)
{
...
static void lyn_sdhci_dwcmshc_dl_cfg(struct sdhci_host *host)
{
    uint32_t tap_max = 0;
	uint16_t clk = 0;
    ...
    switch (host->timing) {
		...
		case MMCSD_TIMING_UHS_SDR50:
	        tap_max = 0x63;		/* For 50MHz clock, 120 Taps are available */
	        break;
		case MMCSD_TIMING_UHS_SDR104:
	    case MMCSD_TIMING_UHS_DDR50:
	    case MMCSD_TIMING_MMC_DDR52:
	        tap_max = 0x60;		/* For 100MHz clock, 60 Taps are available */
	        break;
	    case MMCSD_TIMING_MMC_HS200:
	        tap_max = 0x2b;	//setup=~1.759ns
	        break;
	    case MMCSD_TIMING_MMC_HS400:
			tap_max = 0x23; //setup=~1.453ns
	        break;
	    default:
	        break;
    }

    // 1.Turn-off Sampling Clock Set CLK_CTRL_R.SD_CLK_EN=0
    clk = sdhci_readw(host, SDHCI_CLOCK_CONTROL);
    clk &= ~(SDHCI_CLOCK_CARD_EN);
    sdhci_writew(host, clk, SDHCI_CLOCK_CONTROL);

    sdhci_writeb(host, 0x08, DWC_MSHC_SMPLDL_CNFG);	   //0x320
    sdhci_writeb(host, 0x08, DWC_MSHC_ATDL_CNFG);      //0x321

    sdhci_writeb(host, 0x10, DWC_MSHC_SDCLKDL_CNFG);   //BYPASSMODE
    sdhci_writeb(host, tap_max, DWC_MSHC_SDCLKDL_DC);
    rt_hw_us_delay(5);
    sdhci_writeb(host, 0x00, DWC_MSHC_SDCLKDL_CNFG);   //DLMODE

	clk |= SDHCI_CLOCK_CARD_EN;
	sdhci_writew(host, clk, SDHCI_CLOCK_CONTROL);

	LOG_D("enhanced_strobe=%d host->timing=%d CLOCK_CONTROL(0x2c)=0x%x SDCLK_DC(0x31e)=0x%x SMPLDL_CFG(0x320)=0x%x ATDL_CFG(0x321)=0x%x\n",
		host->enhanced_strobe, host->timing, sdhci_readw(host, SDHCI_CLOCK_CONTROL), sdhci_readb(host, DWC_MSHC_SDCLKDL_DC),
		sdhci_readb(host, DWC_MSHC_SMPLDL_CNFG), sdhci_readb(host, DWC_MSHC_ATDL_CNFG));
}
...
static int dwcmshc_phy_init(struct sdhci_host* host)
{
    ...
    /* Disable the clock */
    clk = sdhci_readw(host, SDHCI_CLOCK_CONTROL);
    clk &= ~SDHCI_CLOCK_INT_EN;     //Clock Disable
    sdhci_writew(host, clk, SDHCI_CLOCK_CONTROL);
    sdhci_writeb(host, 0x10, DWC_MSHC_SDCLKDL_CNFG);   //BYPASSMODE
    sdhci_writeb(host, 0x7f, DWC_MSHC_SDCLKDL_DC);
    sdhci_writeb(host, 0x00, DWC_MSHC_SDCLKDL_CNFG);   //DLMODE

    // 1. cclk_tx Enable
    clk = sdhci_readw(host, SDHCI_CLOCK_CONTROL);
    clk |= SDHCI_CLOCK_INT_EN;      //Clock Enable
    sdhci_writew(host, clk, SDHCI_CLOCK_CONTROL);
    ...
    sdhci_writeb(host, 0x11, MSHC_CTRL_R);
...
__attribute__((unused)) static void sdhic_error_recovery(struct sdhci_host* sdhci_host)
{
    uint32_t status;
...
static void sdhci_send_command(struct sdhci_host* sdhci_host, struct sdhci_command* command, rt_bool_t enDMA)
{
    ...
    if (sdhci_data != RT_NULL)
    {
        ...
        LOG_D("start_addr=0x%lx, dma_addr=0x%lx\n", start_addr, dma_addr);
        sdhci_data->dma_addr = dma_addr;
        sdhci_writel(sdhci_host, dma_addr, SDHCI_ADMA_ADDRESS);
        sdhci_writel(sdhci_host, dma_addr >> 32, SDHCI_ADMA_ADDRESS_HI);
#endif
        LOG_D("blockSize=%d, blockCount=%d\n", sdhci_data->blockSize, sdhci_data->blockCount);
        sdhci_writew(sdhci_host, SDHCI_MAKE_BLKSZ(7, sdhci_data->blockSize), SDHCI_BLOCK_SIZE);
        sdhci_writew(sdhci_host, sdhci_data->blockCount, SDHCI_BLOCK_COUNT32);
    }
...
static rt_err_t sdhci_wait_command_done(struct sdhci_host* sdhci_host, struct sdhci_command* command, rt_bool_t executeTuning)
{
    ...
    // err = rt_event_recv(&sdhci_host->event, SDHCI_INT_ERROR | SDHCI_INT_RESPONSE,
    //     RT_EVENT_FLAG_OR | RT_EVENT_FLAG_CLEAR, rt_tick_from_millisecond(1000) /*RT_WAITING_FOREVER*/, &event);
    // if (err != RT_EOK) {
...
static rt_err_t sdhci_transfer_data_blocking(struct sdhci_host* sdhci_host, struct sdhci_data* data, rt_bool_t enDMA)
{
    ...
    while (1)
    {
        ...
        {
            rt_ubase_t dma_addr = data->dma_addr;
...
static void sdhci_init(struct sdhci_host* host)
{
    ...
    ctrl2 |= SDHCI_CTRL_V4_MODE | SDHCI_CTRL_64BIT_ADDR;
...
static void lx_mmc_request(struct rt_mmcsd_host* host, struct rt_mmcsd_req* req)
{
...
static void lx_mmc_clock_freq_change(struct sdhci_host* host, uint32_t clock)
{
    uint32_t div, val = 0;
...
static void sdhci_set_uhs_signaling(struct sdhci_host *host, unsigned timing)
{
    rt_uint16_t ctrl_2;
    ...
    if ((timing == MMCSD_TIMING_MMC_HS200) || (timing == MMCSD_TIMING_UHS_SDR104))
        ctrl_2 |= SDHCI_CTRL_UHS_SDR104;
    else if (timing == MMCSD_TIMING_UHS_SDR12)
        ctrl_2 |= SDHCI_CTRL_UHS_SDR12;
    else if (timing == MMCSD_TIMING_UHS_SDR25)
        ctrl_2 |= SDHCI_CTRL_UHS_SDR25;
    else if (timing == MMCSD_TIMING_UHS_SDR50)
        ctrl_2 |= SDHCI_CTRL_UHS_SDR50;
    else if ((timing == MMCSD_TIMING_UHS_DDR50) || (timing == MMCSD_TIMING_MMC_DDR52))
        ctrl_2 |= SDHCI_CTRL_UHS_DDR50;
    else if (timing == MMCSD_TIMING_MMC_HS400)
        ctrl_2 |= SDHCI_CTRL_HS400; /* Non-standard */
    sdhci_writew(host, ctrl_2, SDHCI_HOST_CONTROL2);
}

static void lx_set_iocfg(struct rt_mmcsd_host* host, struct rt_mmcsd_io_cfg* io_cfg)
{
    struct sdhci_host* mmcsd;
    ...
    if (!mmcsd->is_emmc_card) {
        if (io_cfg->signal_voltage == MMCSD_SIGNAL_VOLTAGE_180) {
            uint32_t reg = sdhci_readw(mmcsd, SDHCI_HOST_CONTROL2);
            reg |= SDHCI_CTRL_VDD_180;
            sdhci_writew(mmcsd, reg, SDHCI_HOST_CONTROL2);
        }

        sdhci_set_uhs_signaling(mmcsd, mmcsd->timing);
    }

    lx_mmc_clock_freq_change(mmcsd, sdhci_clk);
    ...
    emmc_ctrl = sdhci_readl(mmcsd, EMMC_CTRL_R);
    if (mmcsd->is_emmc_card)
        emmc_ctrl |= CARD_IS_EMMC;
    else
        emmc_ctrl &= ~CARD_IS_EMMC;

    sdhci_writel(mmcsd, emmc_ctrl, EMMC_CTRL_R);
}

static void lx_enable_sdio_irq(struct rt_mmcsd_host* mmcsd_host, rt_int32_t en)
{
...
void lx_sdhci0_reset(int value)
{
...
rt_int32_t lx_sdhci_init(void)
{
    ...
    sdhci_host0->mapbase = (void*)rt_ioremap((void*)SDEMMC0_BASE, 0x40000);
    sdhci_host0->index = 0;
    ...
    mmcsd_host0->freq_max = 200000000;
#ifdef BSP_SDIO0_EMMC
    strncpy(mmcsd_host0->name, "emmc", sizeof(mmcsd_host0->name) - 1);
    mmcsd_host0->flags = MMCSD_BUSWIDTH_8 | MMCSD_MUTBLKWRITE | MMCSD_SUP_HIGHSPEED | MMCSD_SUP_HS200 | MMCSD_SUP_SDIO_IRQ;
#else
    ...
    rt_hw_interrupt_install(IRQN_SD0, sdhci_irq, sdhci_host0, mmcsd_host0->name);
...
INIT_DEVICE_EXPORT(lx_sdhci_init);